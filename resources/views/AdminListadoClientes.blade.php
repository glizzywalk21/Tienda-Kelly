...
<body class="min-h-screen flex flex-col bg-gradient-to-br from-indigo-50 via-blue-50 to-white text-gray-800">
  ...
  <nav class="hidden md:flex px-8 py-4 bg-white items-center justify-between shadow-lg sticky top-0 z-50">
    ...
    <div class="flex gap-8">
      ...
      <a href="{{ route('reservations.index') }}" class="font-medium uppercase text-sm hover:text-indigo-600 transition">Reservas</a>
      <a href="{{ route('AdminProfileVista') }}" class="font-semibold uppercase text-sm border border-indigo-600 text-indigo-600 px-3 py-1 rounded-md hover:bg-indigo-600 hover:text-white transition">
        Perfil
      </a>
    </div>
  </nav>
      ...
      <section id="cards" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        @foreach ($clientes as $cliente)
          <article
            ...
            data-email="{{ strtolower($cliente->usuario ?? '') }}"
              ...
              <div class="flex items-center gap-4 w-full">
                <div class="relative h-16 w-16 shrink-0 overflow-hidden rounded-xl ring-1 ring-gray-200">
                  <img
                    ...
                    alt="Avatar de {{ $cliente->nombre }} {{ $cliente->apellido }}"
  ...
  <script>
    ...
    // Filtro cliente (nombre, correo, teléfono)
