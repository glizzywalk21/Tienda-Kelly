  ...
  <section>
    ...
    <div class="bottom-bar fixed bottom-[1%] left-0 right-0 z-[100] flex justify-center md:hidden">
      <div class="bg-gray-900 rounded-2xl w-64 h-14 flex justify-around">
        <div class="flex items-center"><a href="{{ route('admin.index') }}"><img class="w-6"
              src="{{ asset('imgs/admin.home.nav.png') }}" alt="Home"></a></div>
        <div class="flex items-center"><a href="{{ route('admin.vendedores') }}"><img class="w-6"
              src="{{ asset('imgs/admin.sellers.nav.png') }}" alt="Sellers"></a></div>
        <div class="flex items-center"><a href="{{ route('admin.clientes') }}"><img class="w-6"
              src="{{ asset('imgs/admin.users.nav.png') }}" alt="Users"></a></div>
        <div class="flex items-center"><a href="{{ route('AdminProfileVista') }}"><img class="w-6"
              src="{{ asset('imgs/UserIcon.png') }}" alt="Perfil"></a></div>
      </div>
    </div>
    ...
    <div class="max-w-xl mx-auto mt-12 px-4 pb-24">
      ...
      <form method="POST" action="{{ route('admin.actualizarvendedor', ['id' => $vendedor->id]) }}"
        ...
        <div class="mb-6">
          ...
          <input required type="email" name="usuario" id="usuario" value="{{ old('usuario', $vendedor?->usuario) }}"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-2">
          <div>
            <label for="password" class="block text-sm font-medium text-gray-700 mb-1">Nueva contraseña
              (opcional)</label>
            <input type="password" maxlength="8" name="password" id="password" placeholder="Mín. 8 caracteres"
              class="w-full border rounded-md px-4 py-2 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-300">
            @error('password')
              <div class="text-red-500 text-xs mt-1"><strong>{{ $message }}</strong></div>
            @enderror
        ...
        <div class="mb-6">
          ...
          <input required type="text" name="nombre" id="nombre" value="{{ old('nombre', $vendedor?->nombre) }}"
            class="w-full border rounded-md px-4 py-2 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-300">
          @error('nombre')
            <div class="text-red-500 text-xs mt-1"><strong>{{ $message }}</strong></div>
          @enderror
        ...
        <!-- Nombre del local -->
        <div class="mb-6">
          <label for="nombre_del_local" class="block text-sm font-medium text-gray-700 mb-1">Nombre del local
            (público)</label>
        ...
        <div class="mb-6">
          ...
          <input required type="text" name="telefono" id="telefono" value="{{ old('telefono', $vendedor?->telefono) }}"
