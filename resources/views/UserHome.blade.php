...
<script>
  setInterval(async () => {
    try {
      const res = await fetch("{{ route('usuarios.index') }}", { method: "HEAD" });
...
<body class="bg-gradient-to-br from-indigo-100 via-blue-100 to-white text-gray-800">
  ...
  <header class="mt-10 px-6 flex flex-col md:flex-row justify-between items-center gap-6">
    ...
    <div class="animate-fadeInUp delay-400">
      <a href="{{ route('UserProfileVista') }}" title="Ir a mi perfil">
        <img class="rounded-full w-20 h-20 md:w-24 md:h-24 border-4 border-indigo-500 shadow-lg"
          src="{{ $u?->avatar_url ?? asset('images/default-avatar.jpg') }}" alt="Foto Usuario" loading="lazy"
          decoding="async" onerror="this.src='{{ asset('images/default-avatar.jpg') }}'">
      </a>
    </div>
  </header>
  ...
  <section class="mt-6 h-[60vh] rounded-xl shadow-inner mx-4 overflow-hidden">
    <img src="{{ asset('images/indexTK.jpg') }}" alt="Imagen principal"
  ...
  @foreach ($mercadoLocals as $mercadoLocal)
    ...
    <section class="my-12">
      <div
        class="flex flex-col md:grid md:grid-cols-2 gap-8 items-center max-w-6xl mx-auto rounded-xl overflow-hidden shadow-lg card-hover {{ $mercadoLocal->id % 2 == 0 ? 'bg-indigo-900 text-white' : 'bg-white' }}">
        ...
        <div class="p-8 space-y-5">
          ...
          <a href="{{ route('usuarios.mercado', $mercadoLocal->id) }}">
            <button
              class="mt-4 w-full px-6 py-3 rounded-xl bg-gradient-to-r from-indigo-500 to-blue-500 text-white font-semibold shadow-lg hover:scale-105 transition">
              Ver Mercado
            </button>
          </a>
        </div>
  ...
  <section class="mt-12 px-6">
    ...
    <div class="grid gap-8 md:grid-cols-3">
      @foreach ($vendedors->take(3) as $vendedor)
        ...
        @endphp
        <div
          class="bg-white shadow-md rounded-xl overflow-hidden hover:shadow-xl transition transform hover:-translate-y-2 card-hover">
          ...
          <div class="p-6">
            ...
            <a href="{{ route('usuarios.vendedor', $vendedor->id) }}"
              class="flex gap-2 items-center text-indigo-600 font-medium mt-2 hover:underline">
